...
$ctx = !empty($_REQUEST['ctx']) ? preg_replace('/[^a-zA-Z0-9_\-]/', '', $_REQUEST['ctx']) : 'web';
...
$path = $msGiftCards->config['processorsPath'] . 'web/';
...
if (is_string($result)) {
    ...
    if ($trimmed !== '' && ($trimmed[0] === '{' || $trimmed[0] === '[')) {
        ...
        if (is_array($decoded)) {
            $normalized = $decoded;
            if (isset($decoded['object']) && !isset($decoded['data']) && is_array($decoded['object'])) {
                $normalized['data'] = $decoded['object'];
                unset($normalized['object']);
            }
            header('Content-Type: application/json; charset=utf-8');
            exit(json_encode($normalized));
        }
        header('Content-Type: application/json; charset=utf-8');
        exit($result);
    }
